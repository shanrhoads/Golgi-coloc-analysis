...
def infer_nuclei_fromlabel(in_img: np.ndarray, 
                            ...
                            declumping: bool,
                            declump_footprint: int,
                            declump_smallest_obj: int,
                            intensity_threshold: Union[int, float]) -> np.ndarray:
    """
    ...
    fill_filter_method: str
        method for filling holes and removing small objects in nuclei post-processing; either "slice_by_slice" or "3D"
    declumping: bool
        whether to apply watershed declumping after initial segmentation
    declump_footprint: int
        footprint size for erosion step in watershed declumping; this controls how separated the seeds are that are used for watershed declumping, and thus how aggressive the declumping is (i.e. a larger footprint will result in more separated seeds and thus more aggressive declumping)
    declump_smallest_obj: int
        minimum object size for seeds used in watershed declumping; this is used to filter the seeds generated by erosion to remove small objects that may be generated by the erosion step, which can help reduce over-segmentation in the watershed declumping step
    intensity_threshold: int or float
        Threshold value for mean intensity; nuclei objects with mean intensity above this value will be removed.        
    ...
    # PRE_PROCESSING
    ###################                
    nuclei_smoothed =  scale_and_smooth(nuclei,
    ...
    # CORE_PROCESSING
    ###################
    nuclei_object = apply_log_li_threshold(nuclei_smoothed, 
    ...
    if declumping==False:
        ...
        seeds = binary_erosion(nuclei_labels, footprint=[(np.ones((declump_footprint, 1, 1)), 1), (np.ones((1, declump_footprint, 1)), 1), (np.ones((1, 1, declump_footprint)), 1)])
        seeds_refined = label(fill_and_filter_linear_size(seeds, hole_min=0, hole_max=0, min_size=declump_smallest_obj, method="3D"))
        nuclei_labels = watershed(inverse,
                                markers=seeds_refined,
                                connectivity=2,
                                mask=nuclei_labels)
    else:
        raise ValueError("Declumping must be either True or False")
    
    # filter based on intensity of raw nuclei channel; remove any nuclei objects from the nuc_labels_dc array if their mean intensity is above a specified threshold
    nuclei_labels_filtered = filter_nuc_by_intensity(nuclei_labels, nuclei, intensity_threshold=intensity_threshold)
...
def infer_cellmask_fromcomposite(in_img: np.ndarray,
    ...
    ###################
    cellmask_labels = masked_inverted_watershed(struct_img, 
                                                nuclei_labels, 
                                                struct_obj,
                                                method=watershed_method)

    return cellmask_labels
...
def infer_masks(in_img: np.ndarray, 
                ...
                nuc_declumping: bool,
                nuc_footprint_sz: int,
                nuc_seed_small_object_size: int,
                nuc_intensity_threshold: float,
    ...
    nuc_objs = infer_nuclei_fromlabel(in_img,
                                      ...
                                      nuc_declumping,
                                      nuc_footprint_sz,
                                      nuc_seed_small_object_size,
                                      nuc_intensity_threshold)
    
    ######################
    ### infer cellmask ###
    ######################
    cellmask_objs = infer_cellmask_fromcomposite(in_img,
                                                cell_weights,
                                                cell_rescale,
                                                nuc_objs,
                                                cell_median_sz,
    ...
    ### stack masks ###
    ###################
    maskstack = np.stack([cellmask_objs, nuc_objs], axis=0).astype(np.uint8)
